...
const getArticleUrl = (title) => {
  if (!title) return "https://www.constitutionofindia.net/read/";
  const match = title.match(/^Article\s+([0-9]+[A-Za-z]?)/i);
  if (match) {
    return `https://www.constitutionofindia.net/articles/article-${match[1].toLowerCase()}/`;
  }
  return "https://www.constitutionofindia.net/read/";
};

const normaliseArticle = (a) => {
  return {
    ...a,
    tags: typeof a.tags === 'string' ? a.tags.split(',').map(s => s.trim()) : (a.tags || [])
  };
};
    ...
    api.get(`/articles?${params}`)
      .then(res => {
        ...
        if (Array.isArray(content) && content.length > 0) {
          setArticles(content.map(normaliseArticle));
              ...
              >
                ...
                {expanded === a.id && (
                  <div className="article-body">
                    ...
                    {/* Read Full Article button — redirects to official Constitution of India site */}
                    <div style={{ marginTop: '16px' }}>
                      <a
                        href={getArticleUrl(a.title)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="btn btn-primary"
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', padding: '10px 20px', textDecoration: 'none' }}
                        onClick={e => e.stopPropagation()}
                      >
                        📖 Read Full Article on Constitution of India →
                      </a>
                    </div>
                  </div>
                )}
