...
// Normalise a question from the backend (optionA/B/C/D fields) into the
// shape the UI expects: { options: [...], correctOption: 0-based index }
const normalise = (q) => {
  if (Array.isArray(q.options)) return q;
  return {
    ...q,
    options: [q.optionA, q.optionB, q.optionC, q.optionD].filter(opt => opt != null),
  };
};
...
export default function Quizzes() {
  ...
  useEffect(() => {
    setLoading(true);
    api.get('/quizzes/generate?countPerCategory=4')
      .then(res => {
        const raw = res.data?.content || res.data;
        if (Array.isArray(raw) && raw.length > 0) {
          setQuestions(raw.map(normalise));
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setServerScore(localScore());
    } finally {
      setSubmitLoading(false);
      setSubmitted(true);
      showToast('✅ Quiz scored and saved to profile!');
    }
  };
